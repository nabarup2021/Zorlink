  ...
  return (
    <div className="grid lg:grid-cols-[230px_1fr] gap-6 items-start">
      <aside className="rounded-3xl border border-white/[0.06] bg-[#121019] p-4 space-y-2 lg:sticky lg:top-24">
        ...
        <button onClick={() => setCategory('all')} className={`w-full rounded-2xl px-3 py-2.5 text-left text-xs font-bold ${category === 'all' ? 'bg-azelia-accent text-white' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
          All · {azeliaCommands.length}
        </button>
        {azeliaCommandCategories.map((item) => {
          const count = azeliaCommands.filter((command) => command.category === item).length;
          return (
      ...
      <section className="space-y-5">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-white/10 bg-[#121019] py-3.5 pl-11 pr-4 text-xs text-white outline-none focus:border-azelia-accent/60 placeholder:text-gray-600"
          />
        </div>
          ...
          {filtered.map((command) => (
            <article key={`${command.category}-${command.name}`} className="rounded-3xl border border-white/[0.06] bg-[#121019] p-5 hover:border-azelia-accent/25 transition-all">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <span className="rounded-lg bg-blue-500/10 px-2 py-1 text-[10px] font-bold text-blue-300">Cooldown enforced</span>
